...
void p2b_moe_batched_kernel(
    ...
    int hidden,
    int inter,
    float swiglu_limit)   // LNA-LAB: DSV4 clamped SwiGLU (config swiglu_limit, e.g. 10.0); <= 0 disables
{
    ...
    // Phase 3: SwiGLU activation + Down input Hadamard across all active experts
    {
        ...
        for (int j = tid; j < total_elements; j += total_threads) {
            ...
            if (swiglu_limit > 0.0f) {
                // LNA-LAB: DeepSeek-V4 activation = silu(min(g, L)) * clamp(u, -L, L)
...
at::Tensor p2b_fused_moe_cuda(const at::Tensor& x, at::Tensor& out,
    ...
    int64_t kd, bool mcg, double swiglu_limit) {
    ...
    if (kg == 2) launch_moe_batched<2>(x, gt, gu, gv, ut, uu, uv, dt, du, dv, ids, rw, out, gate, up, down, had_gate, had_up, had_down, accum, e, m, hidden, inter, static_cast<float>(swiglu_limit));
    else if (kg == 3) launch_moe_batched<3>(x, gt, gu, gv, ut, uu, uv, dt, du, dv, ids, rw, out, gate, up, down, had_gate, had_up, had_down, accum, e, m, hidden, inter, static_cast<float>(swiglu_limit));
    else if (kg == 4) launch_moe_batched<4>(x, gt, gu, gv, ut, uu, uv, dt, du, dv, ids, rw, out, gate, up, down, had_gate, had_up, had_down, accum, e, m, hidden, inter, static_cast<float>(swiglu_limit));
