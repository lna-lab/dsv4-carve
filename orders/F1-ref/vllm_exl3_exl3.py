...
def pin_exl3_expert_map(
    ...
    # LNA-LAB: on vLLM builds where `expert_map` is a read-only property (RoutedExperts,
    # 0.28 dev337), keep the pinned copy in a private attribute instead of assigning it.
    pinned = getattr(layer, "_exl3_pinned_expert_map", None)
    if pinned is not None and pinned.device == device:
        return pinned
    ...
    if emap.device != device or emap.dtype != torch.long:
        ...
        try:
            ...
            pass
    layer._exl3_pinned_expert_map = emap
    return emap
...
def _apply_native_fused_moe(
    ...
    if not isinstance(ptrs, dict):
        return None
    # LNA-LAB diagnostic: the pointer tables are raw addresses captured at load time.
    # If the expert tensors were re-wrapped/moved afterwards the table is stale while the
    # exllamav3 path (which holds tensor refs) stays correct. Verify once per layer.
    if not getattr(layer, "_exl3_ptrs_verified", False):
        try:
            cur = torch.tensor([int(pk["gate"].trellis.data_ptr()) for pk in inners], dtype=torch.int64, device=ptrs["gate_trellis"].device)
            same = bool(torch.equal(cur, ptrs["gate_trellis"]))
            logger.warning("EXL3 native ptr table check %s: %s (n_exp=%d)", getattr(layer, "prefix", "?"), "OK" if same else "STALE -> rebuilding", len(inners))
            if not same:
                build_exl3_fused_state(layer, inners)
                ptrs = layer._exl3_ptrs
        except Exception as exc:
            logger.warning("EXL3 native ptr table check failed: %s", exc)
        layer._exl3_ptrs_verified = True
    required = (
        "gate_trellis",
        "gate_suh",
        "gate_svh",
    ...
    n_exp = len(inners)
    # LNA-LAB: map_topk_to_local returns a FLAT (T*K,) tensor; the row loop below indexes
    # [row] and needs (T, K). Without this reshape each row got a single expert id
    # (scalar) and the kernel computed one expert per token -> silent garbage under EP.
    ...
    for row in range(int(x2d.shape[0])):
        result = fn(
            ...
            k,
            True,
            float(limit),   # LNA-LAB: DSV4 swiglu_limit (10.0); the stock kernel had plain silu
        )
    ...
    out32 = native_out.to(dtype=torch.float32)
    # LNA-LAB diagnostic: LNA_EXL3_NATIVE_SELFCHECK=N compares the first N native calls
    # per layer against the Python LinearEXL3 loop on the SAME inputs and logs rel_err.
    n_chk = int(os.environ.get("LNA_EXL3_NATIVE_SELFCHECK", "0") or 0)
    if n_chk > 0:
        done = int(getattr(layer, "_exl3_selfcheck_done", 0))
        if done < n_chk:
            layer._exl3_selfcheck_done = done + 1
            try:
                ref = apply_exl3_python_loop(x2d, ids, weights, inners, expert_map, limit)
                rel = ((out32 - ref).norm() / (ref.norm() + 1e-6)).item()
                # rerun the native kernel on identical inputs: a race shows as rerun != out
                out_b = torch.empty_like(xh)
                for row in range(int(x2d.shape[0])):
                    fn(xh[row:row+1], out_b[row:row+1], ptrs["gate_trellis"], ptrs["gate_suh"], ptrs["gate_svh"],
                       ptrs["up_trellis"], ptrs["up_suh"], ptrs["up_svh"], ptrs["down_trellis"], ptrs["down_suh"], ptrs["down_svh"],
                       safe_ids[row], safe_weights[row], k, k, k, True, float(limit))
                torch.cuda.synchronize()
                rel_b = ((out_b.float() - ref).norm() / (ref.norm() + 1e-6)).item()
                rel_ab = ((out_b.float() - out32).norm() / (out32.norm() + 1e-6)).item()
                dump_dir = os.environ.get("LNA_EXL3_NATIVE_DUMP", "")
                if dump_dir and rel > 0.5 and not getattr(layer, "_exl3_dumped", False):
                    layer._exl3_dumped = True
                    os.makedirs(dump_dir, exist_ok=True)
                    torch.save({"prefix": getattr(layer, "prefix", "?"), "x": x2d.detach().cpu(), "ids": ids.detach().cpu(), "weights": weights.detach().cpu(),
                                "local": local.detach().cpu(), "safe_ids": safe_ids.detach().cpu(), "safe_weights": safe_weights.detach().cpu(),
                                "ref": ref.detach().cpu(), "out": out32.detach().cpu(), "out_b": out_b.float().detach().cpu(), "k": k, "limit": float(limit),
                                "expert_map": None if expert_map is None else expert_map.detach().cpu(),
                                "gate_trellis": [pk["gate"].trellis.detach().cpu() for pk in inners], "gate_suh": [pk["gate"].suh.detach().cpu() for pk in inners], "gate_svh": [pk["gate"].svh.detach().cpu() for pk in inners],
                                "up_trellis": [pk["up"].trellis.detach().cpu() for pk in inners], "up_suh": [pk["up"].suh.detach().cpu() for pk in inners], "up_svh": [pk["up"].svh.detach().cpu() for pk in inners],
                                "down_trellis": [pk["down"].trellis.detach().cpu() for pk in inners], "down_suh": [pk["down"].suh.detach().cpu() for pk in inners], "down_svh": [pk["down"].svh.detach().cpu() for pk in inners]},
                               os.path.join(dump_dir, f"native-fail-{os.getpid()}-{done}.pt"))
                    logger.warning("EXL3 native selfcheck: dumped failing case to %s", dump_dir)
                logger.warning("EXL3 native selfcheck rerun: rel_b=%.4f rel_ab(out vs rerun)=%.4f", rel_b, rel_ab)
                logger.warning(
                    "EXL3 native selfcheck %s call=%d rows=%d topk=%d k=%d rel_err=%.4f |x|max=%.1f |ref|max=%.1f |out|max=%.1f nan_out=%s nonlocal=%d/%d",
                    getattr(layer, "prefix", "?"), done, int(x2d.shape[0]), topk, k, rel,
                    x2d.float().abs().max().item(), ref.abs().max().item(), out32.abs().max().item(),
                    bool(torch.isnan(out32).any().item()), int((~valid).sum().item()), int(valid.numel()))
            except Exception as exc:
                logger.warning("EXL3 native selfcheck failed: %r", exc)
    return out32
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def _load_exl3(
        ...
        # LNA-LAB: under expert parallelism (--enable-expert-parallel) vLLM keeps each local
        # expert at FULL width (moe tp_size == 1) and partitions by expert id via expert_map.
        # Narrow by the MoE layer's own tp geometry, never the process-global TP, or the
        # loader slices a full-width expert into 1/tp and the dest shape check fails.
        moe_cfg = getattr(owner, "moe_config", None) if owner is not None else None
        moe_pcfg = getattr(moe_cfg, "moe_parallel_config", None) or getattr(owner, "moe_parallel_config", None)
        for src_cfg in (moe_pcfg, moe_cfg):
            if src_cfg is not None and hasattr(src_cfg, "tp_size") and hasattr(src_cfg, "tp_rank"):
                try:
                    tp_size, tp_rank = int(src_cfg.tp_size), int(src_cfg.tp_rank)
                    break
                except Exception:
                    continue
    ...
    def apply(
        ...
        for i in range(n_shards):
            if i in bf16_shards:
                ...
                # LNA-LAB: calculate dense EXL3 outputs in fp32; cast to the
                # caller's dtype only after all shard outputs are assembled.
