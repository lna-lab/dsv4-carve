...
#include "lna_moe_decode.cuh"
#include "lna_moe_ticket.cuh"
...
at::Tensor p2b_fused_moe(const at::Tensor& x, at::Tensor& out,
                         ...
                         int64_t kg, int64_t ku, int64_t kd, bool mcg, double swiglu_limit) {
    return p2b_fused_moe_cuda(x, out, gt, gu, gv, ut, uu, uv, dt, du, dv, ids, rw, kg, ku, kd, mcg, swiglu_limit);
}

at::Tensor lna_moe_decode(const at::Tensor& x, at::Tensor& out,
                          const at::Tensor& gt, const at::Tensor& gu, const at::Tensor& gv,
                          const at::Tensor& ut, const at::Tensor& uu, const at::Tensor& uv,
                          const at::Tensor& dt, const at::Tensor& du, const at::Tensor& dv,
                          const at::Tensor& ids, const at::Tensor& rw,
                          int64_t bits, double swiglu_limit, const at::Tensor& scratch) {
    return lna_moe_decode_cuda(x, out, gt, gu, gv, ut, uu, uv, dt, du, dv,
                               ids, rw, bits, swiglu_limit, scratch);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("p2b_fused_moe", &p2b_fused_moe,
          ...
          py::arg("swiglu_limit") = 0.0);   // LNA-LAB: DSV4 clamped SwiGLU; 0 = plain silu (GLM)
    m.def("lna_moe_decode", &lna_moe_decode,
          "F1 single-launch routed EXL3 decode", py::arg("x"), py::arg("out"),
          py::arg("gate_trellis_ptrs"), py::arg("gate_suh_ptrs"),
          py::arg("gate_svh_ptrs"), py::arg("up_trellis_ptrs"),
          py::arg("up_suh_ptrs"), py::arg("up_svh_ptrs"),
          py::arg("down_trellis_ptrs"), py::arg("down_suh_ptrs"),
          py::arg("down_svh_ptrs"), py::arg("expert_ids"),
          py::arg("routing_weights"), py::arg("K"),
          py::arg("swiglu_limit"), py::arg("scratch"));
    m.def("lna_moe_scratch_bytes", &lna_moe_scratch_bytes_cuda,
          "Bytes required by the F1 stream scratch handle");
    m.def("lna2_moe_decode", &lna2_moe_decode_cuda,
          "F3: ticket-scheduled small-R routed-expert decode");
    m.def("lna2_moe_scratch_bytes", &lna2_moe_scratch_bytes_cuda, "F3 scratch bytes");
    m.def("lna2_moe_max_rows", &lna2_moe_max_rows_cuda,
          "F3: maximum decode rows this build accepts");
    m.def("lna2_moe_prepare", &lna2_moe_prepare_cuda, "F3 prewarm");
    m.def("lna2_moe_info", &lna2_moe_info_cuda,
          "F3: (sms, resident, team width, teams, threads, ctrl bytes, scratch bytes)");
    m.def("lna_moe_info", &lna_moe_info_cuda,
          "F1: (sms, resident_blocks_per_sm, grid.x slices, grid.z groups, dynamic smem bytes)");
    m.def("lna_moe_prepare", &lna_moe_prepare_cuda,
          "Preflight F1 cooperative launch variant", py::arg("K"),
          py::arg("M_CAP"));
}
