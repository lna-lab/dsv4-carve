...
docker run --ulimit core=0 --cap-add=SYS_PTRACE -e VLLM_DISABLE_SHARED_EXPERTS_STREAM=${SHARED_STREAM_OFF:-1} -v /run/media/tonoken3/DATA1/.tmp/dsv4-cache/root-cache:/root/.cache -v /run/media/tonoken3/DATA1/.tmp/dsv4-cache/tilelang:/root/.tilelang ${EXT_SO:+-v $EXT_SO:/usr/local/lib/python3.12/dist-packages/$(basename ${EXT_SO:-x})} ${PLUGIN_SRC:+-v $PLUGIN_SRC:/usr/local/lib/python3.12/dist-packages/vllm_exl3} ${NATIVE_SO:+-v $NATIVE_SO:/usr/local/lib/python3.12/dist-packages/$(basename ${NATIVE_SO:-x})} ${MOE_KERNEL:+-e VLLM_EXL3_MOE_KERNEL=$MOE_KERNEL} -d --name $NAME --gpus "\"device=$GPUS\"" --shm-size=16g --ipc=host \
  ...
  --kv-cache-dtype ${KVDT:-fp8} --gpu-memory-utilization $UTIL ${EAGER:+--enforce-eager} --compilation-config "${COMPILE:-{\"cudagraph_capture_sizes\":[1,2,4]\}}" --disable-custom-all-reduce ${EP:+--enable-expert-parallel} \
