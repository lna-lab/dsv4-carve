...
docker run --ulimit core=0 -v /run/media/tonoken3/DATA1/.tmp/dsv4-cache/root-cache:/root/.cache -v /run/media/tonoken3/DATA1/.tmp/dsv4-cache/tilelang:/root/.tilelang ${EXT_SO:+-v $EXT_SO:/usr/local/lib/python3.12/dist-packages/$(basename ${EXT_SO:-x})} ${PLUGIN_SRC:+-v $PLUGIN_SRC:/usr/local/lib/python3.12/dist-packages/vllm_exl3} -d --name $NAME --gpus "\"device=$GPUS\"" --shm-size=16g --ipc=host \
